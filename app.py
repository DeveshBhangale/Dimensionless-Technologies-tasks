...
def success():  
    if request.method == 'POST':  
        ...
        f.save("C:/Devesh/Dimentionless/app/static/sample_input_dataset/{}".format(f.filename))  
        f1 = request.files['file1'] 
        f1.save("C:/Devesh/Dimentionless/app/static/sample_input_dataset/{}".format(f1.filename))
        
        
        import xml.etree.ElementTree as ET

        tree = ET.parse("C:/Devesh/Dimentionless/app/static/sample_input_dataset/{}".format(f1.filename))
        root = tree.getroot()
        global name 
        global coordinates
        name,coordinates = [],[]
        ...
        image = cv2.imread("C:/Devesh/Dimentionless/app/static/sample_input_dataset/{}".format(f.filename))
        ...
        cv2.imwrite("C:/Devesh/Dimentionless/app/static/sample_output_image/{}".format(f.filename), image)
...
def download_file():
    return send_file('C:/Devesh/Dimentionless/app/static/abc.csv',as_attachment=True)
...
def getcsv():
    if request.method=='POST':
        ...
        imgname = session.get('imagename')
        coordinates = session.get('coordinates')
        for i in range(len(name)):
            d = datetime.datetime.now().strftime('%Y-%m-%d')
            cur.execute(insert,(str(imgname),str(name[i]),(coordinates[i][0]),(coordinates[i][1]),(coordinates[i][2]),(coordinates[i][3]),d))
        mydb.commit()
        import pandas as pd  
        imgname = []
        ...
        df.to_csv('C:/Devesh/Dimentionless/app/static/abc.csv')
        return render_template('download.html', abc = 'C:/Devesh/Dimentionless/app/static/abc.csv')
